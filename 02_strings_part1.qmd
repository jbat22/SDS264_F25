...
```{r}
str_view(spot_smaller$title, "M")
str_view(spot_smaller$title, "^M") #^ means to find it at the start

str_view(spot_smaller$title, "o")
str_view(spot_smaller$title, "o$") #$ means to find it at the end
```
...
```{r}
str_view(spot_smaller$album_name, "\\d") #gets digits
```
...
```{r}
str_view(spot_smaller$subgenre, "[hp]op") #means or
...
```{r}
str_view(spot_smaller$album_name, "[^\\d]") #not digits

str_view(spot_smaller$album_name, "[^a-zA-Z ]") #not letters or spaces
```
