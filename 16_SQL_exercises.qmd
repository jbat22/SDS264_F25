...
```{sql, connection = con, output.var = "myplottingdata"}
SELECT 
  Year, origin, dest, cancelled, AVG(cancelled) AS prop_cancelled, COUNT(*) AS num_flights
FROM flightdata
WHERE Year = 2023
...
```{r}
ggplot(myplottingdata, aes(x = num_flights, y = prop_cancelled)) +
  geom_point()
```
