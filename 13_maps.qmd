...
Group represents a group of points to connect up - usually a state but may also represent a separate subsection of a state that is not nicely connected to the rest (michigan's UP, long island new york, etc). Order represents the order in which to connect the points. 
...
We were able to tell what areas are represented in one dataset and not the other, or that have different names - alaska and hawaii are only in the vaccine data, and dc is only in the us states data; additionally, new york is called new york state in the vaccine data and new york in the states data. 
...
We get many-to-many because both sets have multiple observations for each state because we make multiple maps, so its going to make a lot. This is what we want! Thus, it is fine. 
...
It means we aren't biased by the land area of a state, so we may consider all states more equal. We also can have an easier time seeing smaller states. 
...
leaflet() |> 
  addTiles() |>   #<1>
  setView(-87.81060236644092, 42.05572802455216, zoom = 17) |>   #<2>
  addPopups(-87.81060236644092, 42.05572802455216, 'My House :D') 
```
...
Leaflets are much more interactable
...
Suspicious of the density numbers - are there really that many people in south dakota?
