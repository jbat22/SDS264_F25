...
for (i in 1:1000) {
  null_dolphins <- tibble(treatment = sample(dolphin_data$treatment),
                       improve = c(rep("Yes", 10), rep("No", 5), 
                                   rep("Yes", 3), rep("No", 12)))
  null_sum <- null_dolphins |>
  group_by(treatment) |>
  summarize(prop_yes = mean(improve == "Yes"))
  simulated_diffs[i] <- null_sum[[2]][2] - null_sum[[2]][1]
}
simulated_diffs
...
num_unique <- vector("double", ncol(iris))
...
```{r}
#make a function called show_missing
show_missing <- function(df, group_vars, summary_vars = everything()) {
  df |> 
    group_by(pick({{ group_vars }})) |> #group by given grouping variables
    summarize(
      #count the number of NAs in each of the given summary variables and ungroup
      across({{ summary_vars }}, \(x) sum(is.na(x))),
      .groups = "drop"
    ) |>
    select(where(\(x) any(x > 0))) #select only columns that had at least one NA
}
#output how many flights grouped by year, month, and day had missing values in each col
