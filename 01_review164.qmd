...
I prefer the first plot because it is easier to compare between states when on the same plot when they are fairly similar, but the second is nice if you are trying to just see what the trend is in every state and maybe doing less comparing. 
...
The dataset would only include counties with a vaccination rate above 95%

c) What would happen if we replaced `mean_white = mean(perc_white, na.rm = TRUE)` with `mean_white = mean(perc_white)`?
It would possibly end up as NA if any of the counties do not have this data

d) What would happen if we removed `group_by(MD_group)`?
The summarize would summarize everything into a single line with 3 columsn for n, mean_perc_vac, and mean_white
